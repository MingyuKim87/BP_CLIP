    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.add_prompt:
            ...
            if not self.text_layer:
                ...
                removed_part = x[x.shape[0] - self.n_ctx_visual:, :, :]
                # Create/configure learnable tokens of this layer
                visual_context = (self.VPT_shallow.expand(x.shape[1], -1, -1).permute(1, 0, 2).half()) #+ 0.2*removed_part
...
class ResidualAttentionBlock_APeX(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, design_details=None,
                 text_layer=False, i=0):
        super().__init__()
        ...
        self.attn_mask = attn_mask
        # This must be consistent with the config file prompt
        self.compound_prompt_nctx = design_details['apex_length']
        if i == 0:
            self.first_layer = True
        else:
            self.first_layer = False
...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, prompts_needed=0,
        ...
        self.current_trainer = current_trainer = design_details['trainer']
        ...
        elif current_trainer == 'MaPLe' or current_trainer == 'MaPLe_OVE_PG':
            self.resblocks = nn.Sequential(
                *[ResidualAttentionBlock_MaPLe(width, heads, attn_mask, design_details, text_layer, i)
                  for i in range(layers)])
            
        elif current_trainer == 'APeX':
            self.resblocks = nn.Sequential(
                *[ResidualAttentionBlock_APeX(width, heads, attn_mask, design_details, text_layer, i)
                  for i in range(layers)])
    ...
    def forward(self, x: torch.Tensor):
        if self.current_trainer == "APeX":
            all_hidden_states = []
            for i in range(len(self.resblocks)):
                resblock = self.resblocks[i]
                nctx = resblock.compound_prompt_nctx
            
                x = resblock(x)
                
                if resblock.text_layer: 
                    all_hidden_states.append(x[0][1+nctx:].mean((0,1)))
                else: 
                    all_hidden_states.append(x[0][:-nctx].mean((0,1)))
...
class VisionTransformer_APeX(nn.Module):
    ...
    def forward(self, x: torch.Tensor, shared_ctx, compound_deeper_prompts):
        ...
        outputs, all_hidden = self.transformer([x, compound_deeper_prompts, 0])  # third argument is counter
        x = outputs[0]
        x = x.permute(1, 0, 2)  # LND -> NLD
    ...
    def __init__(self,
        ...
        if isinstance(vision_layers, (tuple, list)):
            ...
            if trainer == "MaPLe":
                ...
                self.visual = VisionTransformer_APeX(
                    input_resolution=image_resolution,
                    patch_size=vision_patch_size,
                    width=vision_width,
                    layers=vision_layers,
