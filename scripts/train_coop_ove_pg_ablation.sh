...
for dir in "${dirs[@]}"; do
    ...
    if [[ $dir == *"32"* ]]; then
        ...
        CUDA_VISIBLE_DEVICES=0 python eval_cross_all_datasets_fp32.py --gpuids=0 --epoch=5 --lambda1=0.1
        echo "Completed eval_cross_all_datasets_fp32.py in $new_dirs"

        # Run the first command: eval_domain_all_dasets.py
        CUDA_VISIBLE_DEVICES=0 python eval_domain_all_datasets_fp32.py --gpuids=0 --epoch=5 --lambda1=0.1
        ...
        # Run the second command: eval_cross_all_dasets.py
        CUDA_VISIBLE_DEVICES=0 python eval_cross_all_datasets.py --gpuids=0 --epoch=5 --lambda1=0.1
        echo "Completed eval_cross_all_datasets.py in $dir"

        # Run the first command: eval_domain_all_dasets.py
        CUDA_VISIBLE_DEVICES=0 python eval_domain_all_datasets.py --gpuids=0 --epoch=5 --lambda1=0.1
