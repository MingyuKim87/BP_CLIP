...
class PromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        n_ctx = cfg.TRAINER.VPT_COCOOP_OVE_PG.N_CTX
        ctx_init = cfg.TRAINER.VPT_COCOOP_OVE_PG.CTX_INIT
        self.L = cfg.TRAINER.VPT_COCOOP_OVE_PG.L
        self.vpt_type = cfg.TRAINER.VPT_COCOOP_OVE_PG.VPT_TYPE
        ...
        if self.vpt_type == "cocoopvpt":
            ...
            if cfg.TRAINER.VPT_COCOOP_OVE_PG.PREC == "fp16":
                self.meta_net.half()
        else:
...
class CustomCLIP(nn.Module):
    ...
    def forward(self, image, label=None, batch_A=None, lambda_1=None, freezed_model=None, alpha=None, n_cls=None, M=1):
        ...
        # FEED-FORWARD #
        ################
        pred = logits = torch.stack(logits, dim=1) # L x batch x num_class
        
        # knowledge distillation
        learned_pred = freezed_model.model_inference(image).expand((self.L, -1, -1)) 
        if pred.dtype == torch.float32:
            learned_pred = learned_pred.float()
        
        if self.prompt_learner.training:
            # init
            task_losses, kd_losses, kld_losses = 0, 0, 0

            # label information
            label_one_hot = to_one_hot(label, n_cls)
            tile_label_one_hot = torch.tile(label_one_hot.unsqueeze(0), (self.L, 1, 1))
            kappa = tile_label_one_hot - 0.5            
            
            for _ in range(M):
                # ove_pg softmax
                ove_logits = ove_logit(pred, batch_A)
                learned_ove_logits = ove_logit(learned_pred, batch_A)
                ...
                for cls_idx in range(n_cls):
                    ...
                    kappa_c = kappa_c.unsqueeze(-1).expand((-1, -1, n_cls-1))
                    ...
                    # reshape
                    shape = logit.shape
                    logit = logit.reshape(shape[0]*shape[1]*shape[2], -1)
                    kappa_c = kappa_c.reshape(shape[0]*shape[1]*shape[2], -1)
                    omega = omega.flatten()

                    # for posterior f
                    mu_f, Sigma_f = conditional_posterior_weights_2(logit, kappa_c, alpha, omega)
                    
                    # sampling
                    logit_prime = gaussian_rsample(mu_f, Sigma_f)
                ...
                # cross-entropy loss
                task_log_py = self.nll(log_prob_ove_pg_sm, tile_label_one_hot)
                task_score = torch.logsumexp(task_log_py, dim=0) - torch.log(
                    torch.Tensor([self.L]).type_as(logits)
                )
                task_losses += -task_score.mean(dim=-1)
                ...
                if lambda_1 is None or freezed_model is None:
                    ...
                    kd_losses += (learned_pred - pred).pow(2).mean()

                # regularization for prompt distribution
                kld_loss = self.kl_divergence(mu, logvar)
                if kld_loss.ndim > 0:
                    kld_losses += kld_loss.mean()

            return (1/M)*(task_losses + (lambda_1*kd_losses) + (0.001 * kld_losses)), (1/M)*task_losses, (1/M)*lambda_1*kd_losses, (1/M)*0.001*kld_losses
        else:
...
class VPT_CoCoOp_OVE_PG(TrainerX):
    def check_cfg(self, cfg):
        assert cfg.TRAINER.VPT_COCOOP_OVE_PG.PREC in ["fp16", "fp32", "amp"]
    ...
    def build_model(self):
        ...
        if cfg.TRAINER.VPT_COCOOP_OVE_PG.PREC == "fp32" or cfg.TRAINER.VPT_COCOOP_OVE_PG.PREC == "amp":
        ...
        self.lambda_1 = cfg.TRAINER.VPT_COCOOP_OVE_PG.get("LAMBDA_1", None)
        self.alpha = cfg.TRAINER.VPT_COCOOP_OVE_PG.get("ALPHA", None)
        self.M = self.cfg.TRAINER.VPT_COCOOP_OVE_PG.get("M", None)
        self.eps = 1e-8
        assert self.lambda_1 is not None, "LAMBDA_1 must be given in cfg.TRAINER.VPT_COCOOP_OVE_PG.LAMBDA_1"
        assert self.alpha is not None, "ALPHA must be given in cfg.TRAINER.VPT_COCOOP_OVE_PG_PG.ALPHA"
        assert self.M is not None, "M must be given in cfg.TRAINER.VPT_COCOOP_OVE_PG_PG.M (It indicates the number of gibbs samples) "
        ...
        self.scaler = GradScaler() if cfg.TRAINER.VPT_COCOOP_OVE_PG.PREC == "amp" else None
    ...
    def forward_backward(self, batch):
        ...
        prec = self.cfg.TRAINER.VPT_COCOOP_OVE_PG.PREC
        if prec == "amp":
            with autocast():
                total_loss, task_loss, kd_loss, _ = model(image, label, self.batch_A, self.lambda_1, self.freezed_model, self.alpha, self.n_cls, self.M)
            optim.zero_grad()
