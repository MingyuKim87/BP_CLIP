...
class PromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        n_ctx = cfg.TRAINER.VPT_OVE_PG.N_CTX
        ctx_init = cfg.TRAINER.VPT_OVE_PG.CTX_INIT
        self.L = cfg.TRAINER.VPT_OVE_PG.L
        self.vpt_type = cfg.TRAINER.VPT_OVE_PG.VPT_TYPE
        ...
        if self.vpt_type == "cocoopvpt":
            ...
            if cfg.TRAINER.VPT_OVE_PG.PREC == "fp16":
                self.meta_net.half()
...
class VPT_OVE_PG(TrainerX):
    def check_cfg(self, cfg):
        assert cfg.TRAINER.VPT_OVE_PG.PREC in ["fp16", "fp32", "amp"]
    ...
    def build_model(self):
        ...
        if cfg.TRAINER.VPT_OVE_PG.PREC == "fp32" or cfg.TRAINER.VPT_OVE_PG.PREC == "amp":
        ...
        self.lambda_1 = cfg.TRAINER.VPT_OVE_PG.get("LAMBDA_1", None)
        self.alpha = cfg.TRAINER.VPT_OVE_PG.get("ALPHA", None)
        self.M = self.cfg.TRAINER.VPT_OVE_PG.get("M", None)
        self.eps = 1e-8
        assert self.lambda_1 is not None, "LAMBDA_1 must be given in cfg.TRAINER.VPT_OVE_PG.LAMBDA_1"
        assert self.alpha is not None, "ALPHA must be given in cfg.TRAINER.VPT_OVE_PG_PG.ALPHA"
        assert self.M is not None, "M must be given in cfg.TRAINER.VPT_OVE_PG_PG.M (It indicates the number of gibbs samples) "
        ...
        self.scaler = GradScaler() if cfg.TRAINER.VPT_OVE_PG.PREC == "amp" else None
    ...
    def forward_backward(self, batch):
        ...
        prec = self.cfg.TRAINER.VPT_OVE_PG.PREC
        if prec == "amp":
            with autocast():
